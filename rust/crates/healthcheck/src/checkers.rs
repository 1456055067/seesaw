            ...
            Ok(Ok(addrs)) => {
                ...
                if self.expected_ips.is_empty() {
                    ...
                    let found = self
                        .expected_ips
                        .iter()
                        .any(|expected| resolved_ips.contains(expected));
                    ...
                    if found {
                        ...
                        HealthCheckResult::unhealthy(
                            duration,
                            "Expected IP not found in DNS results",
                        )
                    }
                }
            }
...
mod tests {
    ...
    async fn test_tcp_checker_localhost() {
        ...
        let checker = TcpChecker::new("127.0.0.1:1".parse().unwrap(), Duration::from_millis(100));
    ...
    async fn test_dns_checker() {
        let checker = DnsChecker::new("localhost".to_string(), vec![], Duration::from_secs(1));
